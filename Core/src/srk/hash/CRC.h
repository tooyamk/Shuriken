...
namespace srk::hash {
	/**
		...
		CRC8_SSRK_J1850       false           false            0x1D                0xFF                0xFF
		CRC8_SSRK_J1850_ZERO  false           false            0x1D                0x00                0x00
	...
	*/
	...
	private:
		...
		public:
			...
			inline static constexpr uint_t<Bits> SRK_CALL reflectUnsigned(uint_t<Bits> x, size_t wordLength = std::numeric_limits<uint_t<Bits>>::digits) {
	...
	public:
		...
		template<size_t Bits>
		static TableType<Bits> SRK_CALL createTable(uint_t<Bits> polynomial) {
		...
		inline static ReflectionTableType SRK_CALL createReflectionTable() {
		...
		template<size_t Bits>
		inline static constexpr uint_t<Bits> SRK_CALL init(uint_t<Bits> initialValue) {
			return initialValue;
		}

		template<size_t Bits, bool InputReflected>
		static uint_t<Bits> SRK_CALL update(uint_t<Bits> crc, const void* data, size_t len, const TableType<Bits>& table) {
		...
		inline static uint_t<Bits> SRK_CALL finish(uint_t<Bits> crc, uint_t<Bits> finalXorValue) {
		...
		template<size_t Bits>
		inline static uint_t<Bits> SRK_CALL calc(const void* data, size_t len, uint_t<Bits> initialValue, uint_t<Bits> finalXorValue, bool reflectInput, bool ResultReflected, const TableType<Bits>& table) {
	...
	private:
		...
		template<size_t Bits>
		static void SRK_CALL _wordUpdate(uint_t<Bits>& remainder, uint_t<Bits> truncatedDivisor, uint_t<Bits> newDividendBits, size_t wordLength, bool reflect) {
		...
		template<size_t Bits>
		inline static constexpr uint_t<Bits> SRK_CALL _reflectOptionally(uint_t<Bits> x, bool reflect, size_t wordLength = std::numeric_limits<uint_t<Bits>>::digits) {
