...
namespace srk {
	class SRK_CORE_DLL TimeWheel : public std::enable_shared_from_this<TimeWheel> {
	public:
		class SRK_CORE_DLL Timer : public Ref {
		public:
			Timer();
			virtual ~Timer();
			...
			inline uint64_t SRK_CALL getDelay() const {
				return _delay;
			}

			inline size_t SRK_CALL getCount() const {
				return _count;
			}

			inline bool SRK_CALL isStrict() const {
				return _isStrict;
			}
			...
			inline void SRK_CALL doTick(uint64_t tickID) {
				std::scoped_lock lck(_mutex);
		...
		inline uint64_t SRK_CALL getInterval() const {
			return _interval;
		}

		void SRK_CALL startTimer(Timer& timer, uint64_t delay, size_t count, bool strict);
		IntrusivePtr<Timer> SRK_CALL startTimer(uint64_t delay, size_t count, bool strict, const Timer::OnTickFn& fn);

		inline void SRK_CALL stopTimer(Timer& timer) {
			std::scoped_lock lck(_mutex);
		...
		void SRK_CALL tick(uint64_t elapsed, Fn&& fn) {
			{
				std::scoped_lock lck(_mutex);
	...
	private:
		struct Slot {
			...
			void SRK_CALL remove(Timer*& tickingTimer, Timer* t) {
			...
			void SRK_CALL tick(TimeWheel& wheel, bool skip, uint64_t elapsed, Fn&& fn) {
				if (head) {
		...
		inline void SRK_CALL _rollPointer() {
			if (++_curSlotIndex == _numSlots) _curSlotIndex = 0;
		}

		void SRK_CALL _addTimer(Timer& timer, uint64_t delay, uint64_t elapsed, bool strict);
		void SRK_CALL _repeatTimer(Timer& timer, uint64_t elapsed);
		void SRK_CALL _stopTimer(Timer& timer);
