...
namespace srk {
	...
	public:
		...
		inline constexpr size_t SRK_CALL getSize() const {
			return N;
		}
		...
		template<std::convertible_to<T> K>
		inline bool SRK_CALL isEqual(const K& value) const {
			return Math::isEqual(data, value);
		}

		template<std::convertible_to<T> K, typename S>
		inline bool SRK_CALL isEqual(const K& value, const S& tolerance) const {
			return Math::isEqual(data, value, tolerance);
		}

		template<std::convertible_to<T> K>
		inline bool SRK_CALL isEqual(const Vector<N, K>& value) const {
			return Math::isEqual(data, value.data);
		}

		template<std::convertible_to<T> K, typename S>
		inline bool SRK_CALL isEqual(const Vector<N, K>& value, const S& tolerance) const {
	...
	private:
		...
		inline void SRK_CALL _set(const T& value, Args&&... args) {
			if constexpr (I < N) {
				data[I] = value;
