...
namespace aurora {
	...
	public:
		...
		void AE_CALL set33(const float32_t(&m)[3][3]);
		void AE_CALL set33(
		...
		void AE_CALL decomposition(Matrix34* dstRot, float32_t(dstScale[3]) = nullptr) const;
		...
		inline void AE_CALL appendTranslate(const float32_t(&t)[3]);
		inline void AE_CALL prependTranslate(const float32_t(&t)[3]);
		inline void AE_CALL setPosition(const float32_t(&p)[3]);
		inline void AE_CALL setPosition(const Matrix34& m);
		inline void AE_CALL setPosition(const Matrix44& m);
		inline void AE_CALL setPosition(float32_t x, float32_t y, float32_t z);

		inline void AE_CALL prependScale(const float32_t(&s)[3]);

		static void AE_CALL createLookAt(const float32_t(&forward)[3], const float32_t(&upward)[3], Matrix34& dst);
		static void AE_CALL createRotationAxis(const float32_t(&axis)[3], float32_t radian, Matrix34& dst);

		/**
		 * direction(LH):(0, 1, 0) to (0, 0, 1)
		 */
		static void AE_CALL createRotationX(float32_t radian, Matrix34& dst);

		/**
		 * direction(LH):(1, 0, 0) to (0, 0, -1)
		 */
		static void AE_CALL createRotationY(float32_t radian, Matrix34& dst);

		/**
		 * direction(LH):(1, 0, 0) to (0, 1, 0)
		 */
		static void AE_CALL createRotationZ(float32_t radian, Matrix34& dst);
	...
	public:
		...
		inline static void AE_CALL createOrthoLH(float32_t width, float32_t height, float32_t zNear, float32_t zFar, Matrix44& dst) {
			dst.set44(
		...
		inline static void AE_CALL createOrthoOffCenterLH(float32_t left, float32_t right, float32_t bottom, float32_t top, float32_t zNear, float32_t zFar, Matrix44& dst) {
			dst.set44(
		...
		inline static void AE_CALL createPerspectiveFovLH(float32_t fieldOfViewY, float32_t aspectRatio, float32_t zNear, float32_t zFar, Matrix44& dst) {
		...
		inline static void AE_CALL createPerspectiveLH(float32_t width, float32_t height, float32_t zNear, float32_t zFar, Matrix44& dst) {
		...
		inline static void AE_CALL createPerspectiveOffCenterLH(float32_t left, float32_t right, float32_t bottom, float32_t top, float32_t zNear, float32_t zFar, Matrix44& dst) {
			auto zNear2 = zNear * 2.f;
			dst.set44(
	...
	inline void AE_CALL Matrix34::appendTranslate(const float32_t(&t)[3]) {
		data[0][3] += t[0];
		data[1][3] += t[1];
		data[2][3] += t[2];
	}

	inline void AE_CALL Matrix34::prependTranslate(const float32_t(&t)[3]) {
	...
	inline void AE_CALL Matrix34::setPosition(const float32_t(&p)[3]) {
	...
	inline void AE_CALL Matrix34::setPosition(float32_t x, float32_t y, float32_t z) {
		data[0][3] = x;
		data[1][3] = y;
		data[2][3] = z;
	}

	inline void AE_CALL Matrix34::prependScale(const float32_t(&s)[3]) {
