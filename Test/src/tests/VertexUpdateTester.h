		...
		if (app->createWindow(wndStype, u8"", Box2i32(Vec2i32({ 100, 100 }), Vec2i32({ 800, 600 })), false)) {
			...
			if (gml->load(getDLLName("ae-win-gl"))) {
				...
				if (graphics) {
					...
					rtbs.writeMask.set(false, true, true, true);
					...
					evtDispatcher.addEventListener(ApplicationEvent::TICKING, new EventListener(std::function([graphics, vbf, cf, p, ib, bs](Event<ApplicationEvent>& e) {
