...
public:
	virtual int32_t SRK_CALL run() override {
		...
		Matrix3x4f32 m1(nullptr);
		Matrix3x4f32 m2(nullptr);
		Matrix3x4f32 m3(nullptr);

		m2.rotationX(Math::rad(90.0f));

		m3.translation(Vec3f32(1, 2, 3).data);

		Matrix3x4f32 m1world(nullptr);
		m3.append(m2, m1world);

		Vec3f32 p;
		m1world.transformPoint(Vec3f32().data, p.data);

		Matrix3x4f32 m1worldInv(nullptr);
		m2.invert(m1worldInv);

		Matrix3x4f32 m2local(nullptr);
		m1world.append(m1worldInv, m2local);
