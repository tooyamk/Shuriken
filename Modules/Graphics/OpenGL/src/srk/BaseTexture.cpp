...
namespace srk::modules::graphics::gl {
	...
	bool BaseTexture::create(Graphics& graphics, const Vec3uz& dim, size_t arraySize, size_t mipLevels, SampleCount sampleCount,
		...
		if (auto u = (requiredUsage & (~graphics.getTexCreateUsageMask())); u != Usage::NONE) {
			graphics.error("OpenGL Texture::create error : has not support requiredUsage " + StringUtility::toString(std::to_underlying(u)));
			return _createDone(graphics, false);
		}
				...
				case TextureType::TEX1D:
				{
					...
					if (isArray) {
						...
						if (supportTexStorage) {
							...
							if (data) {
								if (TextureUtility::isCompressedFormat(format)) {
									for (size_t i = 0; i < mipLevels; ++i) {
										auto size = TextureUtility::getBytes(format, w);
										...
										w = TextureUtility::getNextMipPixels(w);
									}
								} else {
									for (size_t i = 0; i < mipLevels; ++i) {
										for (size_t j = 0; j < arraySize; ++j) {
						...
						if (supportTexStorage) {
							...
							if (data) {
								if (TextureUtility::isCompressedFormat(format)) {
									for (size_t i = 0; i < mipLevels; ++i) {
										if (data[i]) glCompressedTexSubImage1D(glTexInfo.target, i, 0, w, glTexInfo.internalFormat, TextureUtility::getBytes(format, w), data[i]);
										w = TextureUtility::getNextMipPixels(w);
									}
								} else {
									for (size_t i = 0; i < mipLevels; ++i) {
						...
						if (supportTexStorage) {
							if (sampleCount > 1) {
								...
								if (data) {
									if (TextureUtility::isCompressedFormat(format)) {
										for (size_t i = 0; i < mipLevels; ++i) {
											auto size = TextureUtility::getBytes(format, size2);
											for (size_t j = 0; j < arraySize; ++j) {
						...
						if (supportTexStorage) {
							if (sampleCount > 1) {
								...
								if (data) {
									if (TextureUtility::isCompressedFormat(format)) {
										for (size_t i = 0; i < mipLevels; ++i) {
											if (data[i]) glCompressedTexSubImage2D(glTexInfo.target, i, 0, 0, size2[0], size2[1], glTexInfo.internalFormat, TextureUtility::getBytes(format, size2), data[i]);
											size2 = TextureUtility::getNextMipPixels(size2);
										}
									} else {
										for (size_t i = 0; i < mipLevels; ++i) {
						...
						if (data) {
							if (TextureUtility::isCompressedFormat(format)) {
								for (uint32_t i = 0; i < mipLevels; ++i) {
									if (data[i]) glCompressedTexSubImage3D(glTexInfo.target, i, 0, 0, 0, size3[0], size3[1], size3[2], glTexInfo.internalFormat, TextureUtility::getBytes(format, size3.cast<2>()) * size3[2], data[i]);
	...
	bool BaseTexture::_update(size_t arraySlice, size_t mipSlice, const Box3uz& range, const void* data) {
		...
		{
			if (TextureUtility::isCompressedFormat(format)) {
				glCompressedTexSubImage1D(glTexInfo.target, mipSlice, range.pos[0], range.size[0], glTexInfo.internalFormat, TextureUtility::getBytes(format, range.size[0]), data);
			} else {
		...
		{
			if (TextureUtility::isCompressedFormat(format)) {
				glCompressedTexSubImage2D(glTexInfo.target, mipSlice, range.pos[0], 0, range.size[0], arraySlice, glTexInfo.internalFormat, TextureUtility::getBytes(format, range.size[0]), data);
			} else {
		...
		{
			if (TextureUtility::isCompressedFormat(format)) {
				glCompressedTexSubImage2D(glTexInfo.target, mipSlice, range.pos[0], range.pos[1], range.size[0], range.size[1], glTexInfo.internalFormat, TextureUtility::getBytes(format, range.size.cast<2>()), data);
			} else {
		...
		{
			if (TextureUtility::isCompressedFormat(format)) {
				glCompressedTexSubImage3D(glTexInfo.target, mipSlice, range.pos[0], range.pos[1], 0, range.size[0], range.size[1], arraySlice, glTexInfo.internalFormat, TextureUtility::getBytes(format, range.size.cast<2>()), data);
			} else {
		...
		{
			if (TextureUtility::isCompressedFormat(format)) {
				glCompressedTexSubImage3D(glTexInfo.target, mipSlice, range.pos[0], range.pos[1], range.pos[2], range.size[0], range.size[1], range.size[2], glTexInfo.internalFormat, TextureUtility::getBytes(format, range.size.cast<2>()) * range.size[2], data);
			} else {
