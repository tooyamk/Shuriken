		...
		bool AE_CALL createInit(Graphics& graphics, Usage resUsage, uint32_t resSize, uint32_t dataSize, UINT mipLevels, UINT& cpuUsage, D3D11_USAGE& d3dUsage) {
			if constexpr (!Tex) {
				if ((resUsage & Usage::RENDERABLE) == Usage::RENDERABLE) {
					if ((resUsage & Usage::IGNORE_UNSUPPORTED) == Usage::NONE) {
						graphics.error("d3d11 Buffer:create error : not support Usage::RENDERABLE");
						return false;
					} else {
			...
			if (resCpuRead) {
				if ((resUsage & Usage::UPDATE) == Usage::UPDATE) {
					graphics.error("d3d11 Resource:create error : could not enable Usage::UPDATE and Usage::MAP_READ at same time");
					return false;
				}
				...
				if ((resUsage & Usage::UPDATE) == Usage::UPDATE) {
					graphics.error("d3d11 Resource:create error : could not enable Usage::UPDATE and Usage::MAP_WRITE at same time");
					return false;
				}
			...
			if constexpr (Tex) {
				if ((resUsage & Usage::RENDERABLE) == Usage::RENDERABLE) {
					if (d3dUsage == D3D11_USAGE_DEFAULT) {
						...
						if (resCpuRead) {
							if (resCpuWrite) {
								graphics.error("d3d11 Resource:create error : could not enable Usage::RENDERABLE and Usage::MAP_READ_WRITE at same time");
							} else {
								graphics.error("d3d11 Resource:create error : could not enable Usage::RENDERABLE and Usage::MAP_READ at same time");
							}
						} else if (resCpuWrite) {
							graphics.error("d3d11 Resource:create error : could not enable Usage::RENDERABLE and Usage::MAP_WRITE at same time");
						} else {
							graphics.error("d3d11 Texture:create error : not support Usage::RENDERABLE, internal usage type error");
						}
						return false;
					}
				}
