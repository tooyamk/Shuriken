	...
	void Input::poll() {
		{
			...
			for (size_t i = 0; i < deviceCount; ++i) {
				...
				uint32_t pcbSize = 0;
				GetRawInputDeviceInfoA(dev.hDevice, 0x20000007, nullptr, &pcbSize);

				char* pData = new char[pcbSize + 1];
				pData[pcbSize] = 0;
				GetRawInputDeviceInfoA(dev.hDevice, 0x20000007, pData, &pcbSize);
