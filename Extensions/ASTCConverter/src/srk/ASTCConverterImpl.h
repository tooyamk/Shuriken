...
namespace srk::extensions::astc_converter {
	void SRK_CALL release(astcenc_context* context) {
		if (context) astcenc_context_free(context);
	}

	ByteArray SRK_CALL encode(const Image& img, Vec3<uint8_t> blockSize, ASTCConverter::Preset preset, ASTCConverter::Flags flags, size_t threadCount) {
